...
<div class="container-fluid">
	...
	<div class="row justify-content-center align-items-center p-4" style="box-shadow: 3px 3px 12px rgba(0,0,0,0.8);border-radius: 5px;">
			...
			<div class="form-group col-md-12">
		  	...
		  	<div style="color: red;font-size:35px;"><%=(session.getAttribute("err2") != null) ? (session.getAttribute("err2")) : ""  %></div>
		  	<div style="color: red;font-size:35px;"><%=(session.getAttribute("err3") != null) ? (session.getAttribute("err3")) : ""  %></div>
      		<label for="depositeAmount"><h4>Enter Account Number of Beneficiary:</h4></label>
      		<spr:input class="form-control" id="depositeAmount" path="accNo" />
    		</div>
			</div>
			
			<div class="form-row">
			<div class="form-group col-md-12">
      		<label for="transferAmount"><h4>Enter Amount to be Transfer:</h4></label>
      		<spr:input class="form-control" id="transferAmount" path="accBalance" />
    		</div>
			</div>
			
			<button type="submit" class="btn btn-dark mb-2">Transfer</button>
			<!--<a href="accTransaction.jsp" class="btn btn-primary btn-lg active" role="button" style="background-color:grey;border:none;width:150px; " aria-pressed="true">Service</a>-->
			</spr:form>
			</div>
	</div>
	</div>
